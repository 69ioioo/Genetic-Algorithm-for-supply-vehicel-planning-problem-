...
def Spreard_of_departures(installation_index):
    weekday_circle = copy.deepcopy(week_day)
    weekday_circle.append(1)
    while True:
        install_patt = sorted(np.random.choice(weekday_circle, fq[Inst_index], replace = False))
        if fq[Inst_index] == 2:
            sub_horizon = []
            for i in range(0, len(weekday_circle)-2):
                h = weekday_circle[i:i+3]
                sub_horizon.append(h)
            check_p = []
            for i in range(len(sub_horizon)):
                chk = set(install_patt)&set(sub_horizon[i])
                check_p.append(len(chk))
            if all(map(lambda x: x<=1, check_p)) == True and all(map(lambda x: x>=0, check_p)) == True:
                break
        if fq[Inst_index] == 3:
            sub_horizon = []
            for i in range(0, len(weekday_circle)-2):
            ...
            for i in range(len(sub_horizon)):
                chk = set(install_patt)&set(sub_horizon[i])
                check_p.append(len(chk))
            if all(map(lambda x: x<=2, check_p)) == True and all(map(lambda x: x>=1, check_p)) == True:
                break
    return install_patt
...
depot_capacity = 3
installation_gap_days = 1 # installation每次服務與服務之間的間隔
...
while iteration <= popsize-1:
    ...
    #------------------------ STEP 1: 生成初始 Installation Service Pattern -------------------------------
        ...
        daily_service_limit = vehicle_cap * depot_capacity
        if all(map(lambda x: len(x) <= daily_service_limit, serviceList_day)) == True:
            break
        p=0
    ...
    while True:
        ...
        day_count = 1
        while day_count <= len(week_day): # 檢查每天需要服務的inst數量是否超過當天出航PSV的總容量
            if len(serviceList_day[day_count-1]) == vehicle_cap: # 當天需服務inst數量若與vehicle_cap相同則只需派一艘
                day_demandPSV = int(len(serviceList_day[day_count-1]) // vehicle_cap)
            ...
            else: # 當天需服務inst數量若小於vehicle_cap相同則只需派一艘，大於則需派 n+1艘
        ...
        check_psv = np.unique(np.hstack(PSVservice_day)).tolist()
        if all(check_day) == True and all(check_psv) == True:
            break
