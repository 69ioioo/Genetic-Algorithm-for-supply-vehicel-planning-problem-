...
while iteration < 2:
    ...
    while True:
        Psv_index = 1
        while Psv_index < len(vehicle_number)+1:
            Psv_pattern = sorted(np.random.choice(week_day, np.random.randint(1, len(week_day)+1), replace = False))
            diff = np.diff(Psv_pattern)
            check_SunMon = max(Psv_pattern) - min(Psv_pattern)
            # 限制PSV每次出航的間隔 至少要一天
            if service_gap_days not in diff: # 可以加入限制一艘船至少要出一天以上的班＊＊＊
                if check_SunMon != 6:
                    init_PSVPattern.append(Psv_pattern)
                    Psv_index += 1
            # else:
            #     continue
        check_depotcap = list(map(lambda x: len(x) <= depot_capacity, init_PSVPattern))
        #確保installations需要被服務的天數都有被包含＆ Depot capacity沒有被違反
        if np.unique(np.hstack(init_PSVPattern)).tolist() == Days_InstNeedService and all(check_depotcap) == True:
            break
        else:
            del init_PSVPattern[:]
    ...
    for i in week_day:
        temp_day = []
        for j in range(len(init_PSVPattern)):
            if i in init_PSVPattern[j]:
                temp_day.append(j+1)
        PSVservice_day.append(temp_day)
    for i in range(len(PSVservice_day)):
        if len(PSVservice_day[i]) == 1 and len(serviceList_day[i]) > vehicle_cap:
            if i == 0:
                remained_psv = list(set(vehicle_number) - set(PSVservice_day[i]) - set(PSVservice_day[i+1]) - set(PSVservice_day[len(PSVservice_day)-1]))
            elif i == len(PSVservice_day)-1:
                remained_psv = list(set(vehicle_number) - set(PSVservice_day[i]) - set(PSVservice_day[i-1]) - set(PSVservice_day[0]))
            else:
                remained_psv = list(set(vehicle_number) - set(PSVservice_day[i]) - set(PSVservice_day[i-1]) - set(PSVservice_day[i+1]))
            add_num = len(serviceList_day[i]) // vehicle_cap
            add_psv = list(np.random.choice(remained_psv, add_num, replace = False))
            for j in range(len(add_psv)):
                PSVservice_day[i].append(add_psv[j])
    ...
    individuals_fitness.append(in_fit)    
# 定義 s1 & s2 (Parents)
